...
The success of high-performing players of \textit{GeoGuessr} shows that enough objects, even common ones, have the power to substantially prune a large search space down to just a few, or even one candidate region \cite{Berners-Lee2023}.
...
The most common forumulations of the problem are as qualitative spatial reasoning constraint satisfaction problems, set-based search and as a graph-matching problem. 
...
Set-based approaches to pictorial querying explored matching on the dimensions of \textit{matching similarity}, \textit{contextual similarity}, and \textit{spatial similariy} \cite{Soffer1997} \cite{Soffer1998a}. 
...
Work in \textit{query-by-sketch} is related but distinct area of that typically seeks to match a map area in database by a user sketch-map that is digitised and used as a query. Systems like \textit{Sketchmapia} \cite{Schwering2014} are better suited for retriving areas larger than individual locations. 
