...
\emph{GESTALT} takes a human-centric approach to geospatial search, leveraging human spatial cognition, navigation and interpretation of objects on a map.
...
Reliance on anchoring underpins \emph{GESTALT}'s human-centric approach to last-mile spatial search wherein the goal is to retrieve locations within a region given information about their objects.
...
The success of high-performing players of \textit{GeoGuessr} shows that enough objects, even common ones, have the power to substantially prune a large search space down to just a few, or even one candidate region \cite{BernersLee2023}.
...
\subsection{Spatial and Pictorial Querying}
...
The most common formulations of the problem are as qualitative spatial reasoning constraint satisfaction problems, set-based search and as a graph-matching problem. 
Dylla et. al. provided a comprehensive survey of these qualitative reasoning approaches in 2017 \cite{Dylla2017}. 
Despite all of the approaches abstracting the real-world for reasoning, none of the systems that they surveyed are performant at a production system scale. 
...
Set-based approaches to pictorial querying explored matching on the dimensions of \textit{matching similarity}, \textit{contextual similarity}, and \textit{spatial similarity} \cite{Soffer1997} \cite{Soffer1998a}. 
They were then reshaped as tree-based queries \cite{Soffer1999} and then again as isomorphic subgraph matching problems to strengthen topological matching, at the cost of becoming NP-Complete \cite{Folkers2000}.
Work in \textit{query-by-sketch} is related but distinct area of that typically seeks to match a map area in database by a user sketch-map that is digitized and used as a query. Systems like \textit{Sketchmapia} \cite{Schwering2014} are better suited for retrieving areas larger than individual locations. 
